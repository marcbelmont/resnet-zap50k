...
function M.RandomSizedCrop(size)
    ...
    return function(input)
        ...
        repeat
            ...
            local targetArea = torch.uniform(0.8, 1.0) * area
...
function M.SimpleCrop(gsAmount)
    return function(input)
        -- Grayscale
        if gsAmount > 0 and torch.uniform(0, 1) < gsAmount then
            gs = gs or input.new()
            grayscale(gs, input)
            input = gs
        end
        ...
        if torch.uniform(0, 1) > .5 then
            -- Zoom In
            local zoom = torch.uniform(.9, 1)
            local x = math.floor(torch.uniform(1, width * (1 - zoom)))
            local newWidth = math.floor(width * zoom)
            local y = math.floor(torch.uniform(1, height * (1 - zoom)))
            local newHeight = math.floor(height * zoom)
            out = image.crop(input, x, y, newWidth, newHeight)
        else
            -- Zoom out
            out = torch.Tensor(input:size()):fill(input[{1, 1, 1}])
            local zoom = torch.uniform(.9, 1)
            local newWidth = math.floor(width * zoom)
            local newHeight = math.floor(height * zoom)
            local x = math.floor(torch.uniform(0, width * (1 - zoom)))
            local y = math.floor(torch.uniform(0, height * (1 - zoom)))
            out[{{}, {y + 1, y + newHeight }, {x + 1, x + newWidth}}] = image.scale(input, newWidth, newHeight, "bicubic")
        end
